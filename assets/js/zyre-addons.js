  ...
  $window.on('elementor/frontend/init', function () {
    ...
    var Advance_Tab = function Advance_Tab($scope) {
      var $tabs = $scope.find('.zyre-tabs'),
        ...
        hoverContentDisplay = $scope.hasClass('zyre-tab-content-display-hover--yes'),
      ...
      haObserveTarget($scope[0], function () {
        ...
        if (animationData.placeDecimal && animationData.placeDecimal > 0) {
          ...
          animationData.onStep = function (now) {
            const formatted = insertDecimal(Math.floor(now), animationData.placeDecimal);
    ...
    var ZyreMegaMenu = function ZyreMegaMenu($scope) {
      var widgetId = $scope.data('id');
      var widgetClass = widgetId ? '.elementor-element.elementor-element-' + widgetId : '.zyre-mega-menu';
      ...
      /**
       * Generate and inject responsive CSS for the current mega menu instance.
       *
       * breakpoint:
       *   -1  = always mobile
       *   >0  = responsive breakpoint
       *   null/0 = always desktop
       */
      ...
      // Toggle mobile menu and swap hamburger icons.
      humBurgerBtn.on('click', function (e) {
        var $icon = $(this);
      ...
      // Enable mobile submenu expand/collapse behavior.
      function childrenToggle() {
        ...
        subMenuIndicator.off('click').on('click', function () {
          ...
          } else if (hasMegaMenu && $parentEl.hasClass('zy-mobile-submenu-items')) {
            $parentEl.children('ul.sub-menu:not(.zy-megamenu-panel)').slideToggle();
      ...
      // Initialize mega menu panel sizing and positioning.
      if (megaMenu.length) {
        megaMenu.each(function () {
          ...
          var mmIsFullWidth = $megaMenuItem.hasClass('zy-dropdown-menu-full_width') && $megaMenuItem.hasClass('position_default');
          var mmIsFullWidthRel = $megaMenuItem.hasClass('zy-dropdown-menu-full_width') && $megaMenuItem.hasClass('position_relative');
          ...
          if (mmWidth && mmWidth !== undefined) {
                ...
                $(window)
                  .on(
                    ...
                    debounce(function () {
                      ...
                      var mmPanelWidth = mmPanel.outerWidth();
                      var mmPanelOffset = mmPanel.offset();
                      var overflowLeft = Math.max(0, -mmPanelOffset.left);
                      var overflowRight = Math.max(0, mmPanelOffset.left + mmPanelWidth + 10 - viewportWidth);
                      var overflow = Math.ceil(Math.max(overflowLeft, overflowRight));

                      if (hasMobileMenu) {
                        mmPanel.css('width', '');
                      } else {
                        mmPanel.css({
                          width: mmWidth - overflow + 'px',
                        });
                      }
                    }, 150),
                  )
            ...
            $(window)
              .on(
                'resize',
                debounce(function () {
                  var posleft = Math.floor($megaMenuItem.position().left - $megaMenuItem.offset().left);
                  mmPanel.css({
    ...
    // Image Carousel
    elementorFrontend.hooks.addAction('frontend/element_ready/zyre-image-carousel.default', ($scope) => {
      new ZyreCarousel({ $element: $scope });
    });

    // Logo Carousel
    elementorFrontend.hooks.addAction('frontend/element_ready/zyre-logo-carousel.default', ($scope) => {
      new ZyreCarousel({ $element: $scope });
    });

    // News Ticker
    elementorFrontend.hooks.addAction('frontend/element_ready/zyre-news-ticker.default', ($scope) => {
      new ZyreCarousel({ $element: $scope });
    });
    ...
    OffCanvas.prototype = {
      ...
      init: function init() {
        ...
        if (this.wrap.find('.zyre-offcanvas-content').length > 0) {
          if ($('.zyre-offcanvas-container > .zyre-offcanvas-content-' + this.id).length > 0) {
            $('.zyre-offcanvas-container > .zyre-offcanvas-content-' + this.id).remove();
          }
      ...
      bindEvents: function bindEvents() {
        ...
        $('body').delegate('.zyre-offcanvas-content .zyre-offcanvas-close', 'click', $.proxy(this.close, this));
      ...
      show: function show() {
        $('.zyre-offcanvas-content-' + this.id).addClass('zyre-offcanvas-content-visible');
      ...
      close: function close() {
        ...
        setTimeout(
          $.proxy(function () {
            ...
            $('.zyre-offcanvas-content-' + this.id).removeClass('zyre-offcanvas-content-visible');
          }, this),
          500,
        );
      ...
      _resize: function _resize() {
        ...
        if (off_canvas && off_canvas.length > 0) {
          if (this.buttons_position === 'bottom') {
            ...
            headerHeight = off_canvas.find('.zyre-offcanvas-cart-header').outerHeight(true);
            wrapHeight = off_canvas.find('.zyre-offcanvas-wrap').outerHeight();
            cartTotalHeight = off_canvas.find('.woocommerce-mini-cart__total').outerHeight();
            cartButtonsHeight = off_canvas.find('.woocommerce-mini-cart__buttons').outerHeight();
            cartMessageHeight = off_canvas.find('.zyre-woo-menu-cart-message').outerHeight();
    ...
    elementorFrontend.hooks.addAction('frontend/element_ready/zyre-off-canvas.default', initOffCanvas);
  });
})(jQuery);
